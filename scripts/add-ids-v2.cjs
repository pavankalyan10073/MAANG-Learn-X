...
for (const [file, cfg] of Object.entries(config)) {
  try {
    ...
    // Find items arrays and track item counts per group
    let inItems = false;
    let groupIdx = 0;
    let itemIdx = 0;
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (inItems) {
        // Check if this line has a title: field (start of new item)
        if (!foundTitleInItem && trimmed.startsWith('title:')) {
        ...
        if (foundTitleInItem && (trimmed === '},' || trimmed === '}')) {
          foundTitleInItem = false;
        }

        // Check if we're leaving the items array
        if (trimmed === '],' || trimmed === ']') {
          // Check if next non-empty line suggests end of items
          inItems = false;
        }
      }
